...
export class PaintingToolManager {
  constructor(assetsPath) {
    ...
    document.addEventListener("al-node-spawned", e => {
      ...
      let position = AFRAME.utils.coordinates.parse(
        e.detail.nodes[e.detail.nodes.length - 1].position
      );
      let normal = AFRAME.utils.coordinates.parse(
        e.detail.nodes[e.detail.nodes.length - 1].normal
      );
  ...
  SetupMaterials() {
    ...
    //cache the materials-----
    if (noMaterialFound) {
      this.currentMaterialCache = new MaterialsCache(
        this.texture,
        this.materials
      );
      this.currentMaterialCache.colour = this.mainColour;
      this.currentMaterialCache.lineWidth =
        this.maxlineWidth * this.CurrentWidth;
      this.currentMaterialCache.startTime = Math.random();
      // this.LineMaterial =
      this.currentMaterialCache.lineMaterial = this.materialsHolder.makeMaterial(
        this,
        this.assetsPath + textures[0],
        materials[0]
      );
      if (textures[1]) {
        this.ObjectsMaterial = this.currentMaterialCache.objMaterial = this.materialsHolder.makeMaterial(
          this,
  ...
  UpdateBrush(_group, _Geometry) {
    if (this.strokecount == this.currentstrokecount) {
      ...
      if (_Geometry.vertices.length < 2) {
        this.PaintingToolMeshLinesCache[
          this.PaintingToolMeshLinesCache.length - 1
        ].visible = false;
      } else {
        this.PaintingToolMeshLinesCache[
          this.PaintingToolMeshLinesCache.length - 1
  ...
  getGeometry() {
    ...
    _nodes.forEach(function(node) {
      if (
        int_counter > $this.NodeRangeBegining &&
        int_counter < $this.NodeRangeEnding
      ) {
        ...
        if (drawingdistance) {
          ...
          let norml = new THREE.Vector3(
            node.normal.x,
            node.normal.y,
            node.normal.z
          ); //$this.stringToVector3(node.normal); //should be "ThreeUtils.stringToVector3(node.normal)", but I cannot find how to call it
  ...
  runAnimation(scene, pointerDown) {
    ...
    if ($this.timer >= 1.0) $this.timer -= 1.0;
    if ($this.timer <= -1.0) $this.timer += 1.0;
    //--------run timer---------
    ...
    if (pointerDown) {
      var lineLength =
        ($this.NodeRangeEnding - $this.NodeRangeBegining) /
        ($this.geoCount - 1);
      if (lineLength > 1.0) retrunValue = true;
      // $this.forceTouchUp();//
      $this.currentMaterialCache.lineLength = THREE.Math.clamp(
        lineLength * 0.9,
        0.01,
        1.0
      );
      ...
      if ($this.materialsCache[i].lineMaterial) {
        if ($this.materialsCache[i].lineMaterial.name != "No custom Shader") {
          if ($this.materialsCache[i].lineMaterial.uniforms.time) {
            ...
            $this.materialsCache[i].lineMaterial.uniforms.lineWidth.value =
              $this.materialsCache[i].lineWidth;
            //$this.materialsCache[i].lineMaterial.uniforms.color.value = new THREE.Color(Math.random(), Math.random(), Math.random() );
            $this.materialsCache[i].lineMaterial.uniforms.lengthNormal.value =
              $this.materialsCache[i].lineLength;
          ...
          if (i == $this.materialsCache.length - 1) {
            //only update the curently painting lineMaterial
            $this.materialsCache[i].lineMaterial.uniforms.color.value =
              $this.materialsCache[i].colour;
            $this.materialsCache[i].lineMaterial.uniforms.lineWidth.value =
              $this.materialsCache[i].lineWidth;
            $this.materialsCache[i].lineMaterial.uniforms.lengthNormal.value =
              $this.materialsCache[i].lineLength;
      ...
      for (var j = $this.NodeRangeBegining; j < nodes.length; j++) {
        if (counter-- == 0) {
          ...
          var _DecalElement = new DecalElement(
            ...
            nodes[j],
            paintingToolManager,
            $this.materialsCache[$this.materialsCache.length - 1]
          );
...
export class DecalElement {
  constructor(
    _ObjectsMaterial,
    _node,
    _BrushVariablesInput,
    _parentMaterialCache
  ) {
    if (!_ObjectsMaterial) {
      return;
    }
    ...
    this._scale =
      _BrushVariablesInput.maxelementWidth * this.parentMaterialCache.lineWidth;
    ...
    this.mesh.position.set(
      this._position.x + randomAmoutx,
      this._position.y + randomAmouty,
      this._position.z + randomAmoutz
    );
    this.rotationZ =
