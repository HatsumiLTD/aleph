...
AFRAME.registerComponent("al-painting-tool", {
  ...
  init: function() {
    ...
    rightController.addEventListener("changeBrushSizeAbs", function(evt) {
      if (
        (evt.detail.axis[0] === 0 && evt.detail.axis[1] === 0) ||
        self.previousAxis === evt.detail.axis[1]
      ) {
        return;
      }
      var delta = evt.detail.axis[1] / 300;
      var value = THREE.Math.clamp(
        self.el.getAttribute("brush").size - delta,
        0.0,
        1.0
      );
    ...
    rightController.addEventListener("changeBrushColourAbs", function(evt) {
      ...
      this.CurrentColourHue = THREE.Math.clamp(
        this.CurrentColourHue,
        0.1,
        100.0
      );
      var newColour = String(
        "hsl(" + this.CurrentColourHue + "," + 90 + "%" + "," + 70 + "%" + ")"
      );
  ...
  runAnimation: function() {
    ...
    if (this.state.pointerDown) {
      var lineLength =
        (paintingToolManager.NodeRangeEnding -
          paintingToolManager.NodeRangeBegining) /
        (paintingToolManager.geoCount - 1);
      if (lineLength > 1.0) this.forceTouchUp();
      paintingToolManager.currentMaterialCache.lineLength = THREE.Math.clamp(
        lineLength * 0.9,
        0.01,
        1.0
      );
    ...
    for (var i = 0; i < paintingToolManager.materialsCache.length; i++) {
      if (paintingToolManager.materialsCache[i].lineMaterial) {
        if (
          paintingToolManager.materialsCache[i].lineMaterial.name !=
          "No custom Shader"
        ) {
          if (
            paintingToolManager.materialsCache[i].lineMaterial.uniforms.time
          ) {
            var _time =
              (paintingToolManager.timer +
                paintingToolManager.materialsCache[i].startTime) %
              1.0;
            paintingToolManager.materialsCache[
              i
            ].lineMaterial.uniforms.time.value = _time;
            paintingToolManager.materialsCache[
              i
            ].lineMaterial.uniforms.lineWidth.value =
              paintingToolManager.materialsCache[i].lineWidth;
            ...
            paintingToolManager.materialsCache[
              i
            ].lineMaterial.uniforms.lengthNormal.value =
              paintingToolManager.materialsCache[i].lineLength;
          ...
          paintingToolManager.materialsCache[
            i
          ].lineMaterial.uniforms.color.value =
            paintingToolManager.materialsCache[i].colour;
          paintingToolManager.materialsCache[
            i
          ].lineMaterial.uniforms.lineWidth.value =
            paintingToolManager.materialsCache[i].lineWidth;
  ...
  addDecals: function() {
    ...
    if (paintingToolManager.spacing < 0) {
      var counter = 0;
      for (
        var j = paintingToolManager.NodeRangeBegining;
        j < nodes.length;
        j++
      ) {
        if (counter-- == 0) {
